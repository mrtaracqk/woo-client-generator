...
const main = async (): Promise<void> => {
  ...
  const outputPath = path.resolve(
    process.cwd(),
    options.outputPath ??
      inputPath.replace(/\.snapshot\.json$/, ".manifest.json"),
  );
  ...
  await writeFile(outputPath, stringifyWooRestManifest(manifest), "utf8");

  console.log(
    `Saved ${manifest.operations.length} Woo REST operations to ${path.relative(process.cwd(), outputPath)}`,
  );
};
...
const parseCliOptions = (): CliOptions => {
  ...
  return {
    inputPath: parsed.values.input ?? process.env.WOO_REST_MANIFEST_INPUT,
